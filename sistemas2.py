...
def calcular():
    ...
    try:
        ...
        resultado_label.config(text="Error en la operación")
...
def agregar_digito(digito):
    ...
    if operacion == "":
        ...
        num2 += str(digito)
    resultado_label.config(text=f"Número actual: {num1 if operacion == '' else num2}")

def seleccionar_operacion(op):
    global operacion
    operacion = op
    resultado_label.config(text=f"Operación seleccionada: {operacion}")
